"""Training script for PPO Agent on EW Smart Scan Environment."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train PPO Agent for EW Scan Strategy")
    ...
    parser.add_argument("--save-path", type=str, default="models/rl/ppo_agent.zip", help="Checkpoint save path")
    ...
    print(f"=== Starting PPO Agent Training ({args.total_timesteps} steps) ===")
    agent = PPOAgent(env=train_env, config=ppo_cfg, seed=args.seed)
